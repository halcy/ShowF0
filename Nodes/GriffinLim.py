...
class GriffinLimSynthesis(Node.Node):
    ...
    def add_data(self, dataFrame, data_id=0):
        ...
        bufferIndices = [x + self.inputBufferLength if x < 0 else x for x in bufferIndices]  # Not sure why indices must be positive
        ...
        reconstructedIndices = [x + self.outputBufferLength if x < 0 else x for x in reconstructedIndices]  # Make indices positive. Not sure why

        overlapWindow = scipy.blackman(reconstructedAudioFrames.shape[0])
        self.outputBuffer[reconstructedIndices] += reconstructedAudioFrames  # * overlapWindow
        self.windowBuffer[reconstructedIndices] += overlapWindow  # Why + and not * ???
