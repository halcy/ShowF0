...
class MelFilterBank():
    def __init__(self, specSize, numCoefficients, sampleRate):
        ...
        centerIndices = list([self.freqToBin(math.floor(self.melToFreq(x)), sampleRate, specSize) for x in melFilterEdges])
        ...
        for i in range(numBands):
            ...
            up = (np.array(list(range(start, center))) - start) / k1
            down = (end - np.array(list(range(center, end)))) / k2
    ...
    def fromLogMels(self, melSpectrogram):
        return(self.makeNormal(self.fromMelScale(self.makeNormal(np.exp(melSpectrogram)))))
