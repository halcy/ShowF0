...
class JackAudioSink(Node.Node):
    ...
    def start_processing(self, recurse = True):
        ...
        if self.is_active == False:
            #while True: # TODO turned this off because it seemed to work okay without <- turned it back on lol <- OFF AND WITH RT
    ...
    def add_data(self, samples, data_id = None):
        if samples is None: # HACK: Why does this happen?
            return
        
        #print(self.get_stats())
        
        samples_res = self.resampler.process(samples / (2**15), self.sample_multiplier, end_of_input = False)
